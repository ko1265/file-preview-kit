...
3. Otherwise, re-run the narrow validation set that matters for the current repository baseline:
   - `pnpm.cmd exec tsc -b tsconfig.json`
   - the targeted Web Component and demo regression checks used during signoff
4. `pnpm pack:verify`
5. `pnpm smoke:consumer`
6. Confirm the demo opens with the compact public sample set.
7. Confirm `PUBLIC_DEMO_NOTE.md`, `PUBLIC_LAUNCH_ASSETS.md`, `SCREENSHOT_CHECKLIST.md`, and `LAUNCH_ASSET.svg` match the README framing.
8. Confirm no new Office sample breadth was added beyond the current extraction-oriented scope.
