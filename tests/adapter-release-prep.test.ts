...
describe("adapter release prep", () => {
  it("keeps React/Vue dry-run evidence as historical pre-publish context", async () => {
    ...
    expect(releasePrep).toContain("historical dry-run evidence");
    expect(releasePrep).toContain("The real `1.0.0` publish later completed");
