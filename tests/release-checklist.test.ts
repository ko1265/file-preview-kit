...
describe("release checklist", () => {
  it("covers the final release checks and steps", async () => {
    ...
    expect(checklist).toContain("React tarball");
    expect(checklist).toContain("packed React adapter import");
  });

  it("keeps npm publish docs aligned with the React adapter release path", async () => {
    ...
    expect(publishChecklist).toContain("Do not list `@ko1265/file-preview-kit-react` under \"Published Packages\"");
    ...
    expect(runbook).toContain("stop before `react`");
  });
});
